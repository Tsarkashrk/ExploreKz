...
const register = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (password === cpassword) {
      userService.register({ email, password }, router)
      setEmail('')
      setPassword('')
      setCPassword('')
    }
  ...
  return (
    <div>
      <Head>
        <title>Register | Explore Kazakhstan</title>
        ...
        <div className="md:w-[60%] w-full flex flex-col items-center justify-center min-h-[100vh] px-[30px] py-[30px] relative">
          ...
          <form className="w-full flex flex-col justify-center" onSubmit={handleSubmit}>
            ...
            <div className="w-full relative">
              <input type="email" name="email" className="border px-10 py-2 mb-3 rounded-md w-full" required value={email} onChange={(e) => setEmail(e.target.value)} />
              <HiMail className=" absolute left-4 top-3 text-gray-300 text-xl" />
            </div>
            <label htmlFor="password">Password</label>
            <div className="w-full relative">
              <input type="password" name="password" className="border px-10 py-2 mb-4 rounded-md w-full" minLength={6} required value={password} onChange={(e) => setPassword(e.target.value)} />
              <AiTwotoneLock className=" absolute left-4 top-3 text-gray-300 text-xl" />
              {password === cpassword && cpassword !== '' && <AiFillCheckCircle className=" absolute right-4 top-3 text-green-500 text-xl" />}
            </div>
            <label htmlFor="cpassword">Confirm Password</label>
            <div className="w-full relative">
              <input type="password" name="cpassword" className="border px-10 py-2 mb-4 rounded-md w-full" required minLength={6} value={cpassword} onChange={(e) => setCPassword(e.target.value)} />
              <AiTwotoneLock className=" absolute left-4 top-3 text-gray-300 text-xl" />
              {password === cpassword && cpassword !== '' && <AiFillCheckCircle className=" absolute right-4 top-3 text-green-500 text-xl" />}
            </div>
            {password !== cpassword && <p className="text-red-500 mb-2">Password does not match</p>}
          ...
          <div className="absolute bottom-5 left-5">
            <p className="opacity-50 text-sm">
              <Link href="/">Explore Kazakhstan</Link> &copy; Copyright {new Date().getFullYear()}{' '}
            </p>
          </div>
        </div>
