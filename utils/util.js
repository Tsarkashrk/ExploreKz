...
export const addEventToFirebase = async (id, title, date, time, venue, description, note, flier, router) => {
...
export const getAllEvents = (setEvents, setLoading) => {
  try {
    const q = query(collection(db, 'events')) // No where condition, fetches all events

    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const firebaseEvents = []
      querySnapshot.forEach((doc) => {
        firebaseEvents.push({ id: doc.id, data: doc.data() })
      })
    ...
    return unsubscribe // Ensure unsubscribe is returned for cleanup
  } catch (error) {
...
export const registerAttendee = async (name, email, event_id, setSuccess, setLoading) => {
  setLoading(true)
